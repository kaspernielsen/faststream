 ...
 * {@link #listIterator(int) listIterator} methods are <em>fail-fast</em>:</a> if the list is structurally modified at
 * any time after the iterator is created, in any way except through the iterator's own {@link ListIterator#remove()
 * remove} or {@link ListIterator#add(Object) add} methods, the iterator will throw a
...
public class ArrayList<E> extends AbstractList<E> implements List<E>, RandomAccess, Cloneable, java.io.Serializable {
     ...
     * of this array buffer. Any empty ArrayList with elementData == EMPTY_ELEMENTDATA will be expanded to
    ...
    /**
     * Removes from this list all of the elements whose index is between {@code fromIndex}, inclusive, and
     * {@code toIndex}, exclusive. Shifts any succeeding elements to the left (reduces their index). This call shortens
     * the list by {@code (toIndex - fromIndex)} elements. (If {@code toIndex==fromIndex}, this operation has no
     * effect.)
     *
     * @throws IndexOutOfBoundsException
     ...
     *             if the class of an element of this list is incompatible with the specified collection (<a
     *             href="Collection.html#optional-restrictions">optional</a>)
     * @throws NullPointerException
     *             if this list contains a null element and the specified collection does not permit null elements (<a
     *             href="Collection.html#optional-restrictions">optional</a>), or if the specified collection is null
     ...
     * {@code toIndex}, exclusive. (If {@code fromIndex} and {@code toIndex} are equal, the returned list is empty.) The
     ...
     * {@link Spliterator#ORDERED}. Overriding implementations should document the reporting of additional
    ...
    static final class ArrayListSpliterator<E> implements Spliterator<E> {
        ...
        /*
         * If ArrayLists were immutable, or structurally immutable (no adds, removes, etc), we could implement their
         * spliterators with Arrays.spliterator. Instead we detect as much interference during traversal as practical
         * without sacrificing much performance. We rely primarily on modCounts. These are not guaranteed to detect
         * concurrency violations, and are sometimes overly conservative about within-thread interference, but detect
         * enough problems to be worthwhile in practice. To carry this out, we (1) lazily initialize fence and
         * expectedModCount until the latest point that we need to commit to the state we are checking against; thus
         * improving precision. (This doesn't apply to SubLists, that create spliterators with current non-lazy values).
         ...
         * interference after actions, not before. Further CME-triggering checks apply to all other possible violations
         * of assumptions for example null or too-small elementData array given its size(), that could only have
         * occurred due to interference. This allows the inner loop of forEach to run without any further checks, and
         * simplifies lambda-resolution. While this does entail a number of checks, note that in the common case of
         * list.stream().forEach(a), no checks or other computation occur anywhere other than inside forEach itself. The
         * other less-often-used methods cannot take advantage of most of these streamlinings.
         */
        ...
        ArrayListSpliterator(ArrayList<E> list, int origin, int fence, int expectedModCount) {
            ...
            System.out.println(origin + " " + fence);
        }
