...
/**
 * Test the {@link MapView#size()} operation.
 *
 * @param <K>
 ...
 */
public class SizeIsEmpty<K, V> extends AbstractMapViewRandomTestCase<K, V> {
    ...
    public void isEmpty() {
        ...
        if (expected().isEmpty() != isEmpty) {
            if (isEmpty) {
                fail("Found an empty MapView but expected a MapView with the following entries " + expected());
            } else {
                fail("Expected a MapView with no entries, but contained the following entries " + actual().toMap());
            }
        }
    }
}
