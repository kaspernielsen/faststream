...
/**
 * A test step for {@link Map#keySet()}.
 * 
 * @param <K>
 ...
 */
public class KeySet<K, V> extends AbstractRandomMapTestCase<K, V> {
    ...
    public void keySet() {
        ...
        if (expected().nonStandardOptions.contains(MapRandomTestBuilder.NonStandardOption.KEY_TYPE_RESTRICTION)) {
            set.add(CollectionRandomTestBuilder.NonStandardOption.TYPE_RESTRICTION);
        }
        if (expected().nonStandardOptions.contains(MapRandomTestBuilder.NonStandardOption.NULL_KEYS_UNSUPPORTED)) {
        ...
        ExpectedCollection<K> ec = new ExpectedCollection<>(expected().keyType, expected().generator.keys(), expected().map.keySet(), illegalKeys(), set);
        nested(ec, actual().keySet());
    }
}
