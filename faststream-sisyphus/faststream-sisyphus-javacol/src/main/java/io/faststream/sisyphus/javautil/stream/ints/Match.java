 ...
 */
public class Match extends AbstractRandomIntStreamTestCase {

    @RndTest
    public void allMatch() {
        HashcodeModPredicate<Integer> p = new HashcodeModPredicate<>(random().nextInt(1, 4));
        if (expected().size() == 0) {
    ...
    public void anyMatch() {
        HashcodeModPredicate<Integer> p = new HashcodeModPredicate<>(random().nextInt(2, 14));
        if (expected().size() == 0) {
